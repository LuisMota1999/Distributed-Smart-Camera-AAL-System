...
all_classes_names = os.listdir('../datasets/ToyotaSmartHome/val/')
...
for counter, random_index in enumerate(random_range, 1):
    ...
    video_files_names_list = os.listdir(f'datasets/ToyotaSmartHome/val/{selected_class_name}')

    # Randomly select a video file from the list retrieved from the randomly selected class Directory.
    selected_video_file_name = random.choice(video_files_names_list)

    # Initialize a VideoCapture object to read from the video file.
    video_reader = cv2.VideoCapture(f'./datasets/ToyotaSmartHome/val/{selected_class_name}/{selected_video_file_name}')
